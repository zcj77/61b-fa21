 ...
 *  * int[] B; ... B = Utils.readIntArray(); will, for example, read the input
 *          [ 1, 2, 3 ]
 *       from System.in and set B to point to the 3-element array whose elements are 1, 2, and 3.
 *
 *  * int[][] B; ... B = Utils.readIntArray2(); reads a 2-D array (array of array of ints)
 *       from System.in, in the form
 *           [ [ 1, 2, 3 ], [ 0, 12, 14 ], [ 42 ] ]
 *       Each inner [] segment is one row.  Rows need not be the same length.
 *  * Utils.print(A)
 *       writes a representation of A to System.out. A may be an int[] or int[][].
 ...
 *  * IntList B; ... B = Utils.readIntList(); will, for example, read the input
 *          [ 1, 2, 3 ]
 *       from System.in and set B to point to the 3-element list whose elements are 1, 2, and 3.
 *
 *  * IntListList B; ... B = Utils.readIntListList();
 *       reads a 2-D list (list of list of ints) from System.in, in the form
 *           [ [ 1, 2, 3 ], [ 0, 12, 14 ], [ 42 ] ]
 *       Each inner [] segment is one row.  Rows need not be the same length.
 *
 *  * Utils.reverse(A),
